...
class GenerateQRCode : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        generateQRCode.setOnClickListener{
            count++
            val data = generateCode.text.toString() + count
            if(data.isEmpty())
            {
                Toast.makeText(this@GenerateQRCode,"Введите текст",Toast.LENGTH_LONG).show()
            }
            else
            {
                ...
                qrEncoder = QRGEncoder(data,null,QRGContents.Type.TEXT, this.dimen)
                try {
